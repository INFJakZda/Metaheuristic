...
def compare_pairs(groups1, groups2):
    count = 0
    for i in range(len(groups1)):
        for j in range(len(groups1[i])):
            if groups1[i][j] in groups2[i]:
                count += 1
    return count

def compare_pairs2(groups1, groups2):
    pairs = []
    count = 0
    for group in groups1:
        for i in range(len(group)):
            for j in range(i+1, len(group)):
                pairs.append(str(group[i])+","+str(group[j]))
    for group in groups2:
        for i in range(len(group)):
            for j in range(i+1, len(group)):
                if str(group[i])+","+str(group[j]) in pairs:
                    count += 1
    return count
...
if __name__ == '__main__':
    ...
    #sprawko_5
    compare_pairs_with_the_best(matrix, samples)